...
- **Per-file resume rather than byte-offset, in slice 3.** Everything committed is kept and only the file in flight is redone. Byte-offset resume moves to slice 4b, where FTP makes partial transfers genuinely expensive; over a mounted volume the code to track and validate partial writes carries more corruption risk than the minutes it saves.
